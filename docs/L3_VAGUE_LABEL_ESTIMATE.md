...
| 错配率 **&lt;15%** 且成分未解释 clear 异常 | **信号备忘录**移交基金线（清晰喊单降权/反指候选）；shadow 期继续观察 weak 桶 |
...
**原因（一行）**：两次只读跑数之间生产库**新写入存活已结论**（验证任务持续跑），非口径变更、非 soft-delete 回流；weak **未缩减**（17→18）。若上轮口头出现「276 / weak 15」，276 为中间态未落文档，**15 实为 C 表 weak×empirical 子行**（非 weak 全桶）。

---

## 附录 E：与 L1 shadow 的交互

- `L1_SHADOW_DATA_ERA=pre_other`
- `L3_OTHER_CUTOVER_AT`（empty）
- `reeval_status` 含 `tier_separation` + `era_note`
- 观察期：不改权重，**只读**本备忘录
- 8/26 后：若信号候选，**不改 L1**，仅基金线侧观察

---

## 附录 F：一句话总结（按表归档）

> 8% 错配率 < 15%；双 tier 皆命中≥50% 且收益为负；loud=late 假说；**信号备忘录**移交基金线；**观察期禁动权重**；**不修抽取**；**不部署**。
