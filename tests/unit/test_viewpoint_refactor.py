...
def test_advice_only_reads_dynamic_active_analyzed_viewpoints_without_raw_summary_overlap(test_db):
    target = date.today() - timedelta(days=1)
    _add_viewpoint(test_db, viewpoint_date=target, valid_until=date.today() - timedelta(days=1))
    _add_viewpoint(test_db, viewpoint_date=date.today(), reasoning=None)
    _add_viewpoint(test_db, viewpoint_date=target, summary="原始观点")
    _add_viewpoint(test_db, viewpoint_date=target, source="daily_summary", is_summary=True, summary="每日汇总")

    result = AdviceService(test_db).get_data_for_advice(min_accuracy=0)

    assert [item["summary"] for item in result["viewpoints"]] == ["每日汇总"]


def test_cleanup_preserves_summaries_and_applies_crawler_record_retention(test_db):
    old_day = date.today() - timedelta(days=200)
    summary = _add_viewpoint(test_db, viewpoint_date=old_day, is_summary=True, source="daily_summary")
    original = _add_viewpoint(test_db, viewpoint_date=old_day)
    test_db.add_all([
        CrawlerArticleRecord(
            article_id="adopted-old", source="eastmoney_blog", is_adopted=True,
            viewpoint_id=summary.id, fetched_at=datetime.now() - timedelta(days=181),
        ),
        CrawlerArticleRecord(
            article_id="skipped-old", source="sina_finance", is_adopted=False,
            fetched_at=datetime.now() - timedelta(days=31),
        ),
        CrawlerArticleRecord(
            article_id="adopted-recent", source="eastmoney_blog", is_adopted=True,
            viewpoint_id=summary.id, fetched_at=datetime.now() - timedelta(days=20),
        ),
    ])
    test_db.commit()
    manager = CleanupManager()
    manager._get_db = lambda: _NonClosingSession(test_db)

    viewpoint_result = manager.cleanup_expired_viewpoints()
    record_result = manager.cleanup_crawler_article_records()

    assert viewpoint_result["deleted_viewpoints"] == 1
    assert test_db.get(Viewpoint, original.id) is None
    assert test_db.get(Viewpoint, summary.id) is not None
    assert record_result["deleted_crawler_records"] == 2
    assert test_db.query(CrawlerArticleRecord).one().article_id == "adopted-recent"
